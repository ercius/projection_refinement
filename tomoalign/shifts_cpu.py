...
## --------------------------------------------------------------------------------------------------------
	...
	def imshear_fft(self, img, theta, shear_axis):
		...
		if shear_axis == 0: 
			Ny =  np.tan(np.deg2rad(theta/2)) * (ifftshift(np.arange(-np.floor(N/2), np.ceil(N/2)))/N).reshape(1,N,1)
			Mgrid = 2*(1j)*np.pi * (np.arange(M)-np.floor(M/2)+1)
			Mgrid = Mgrid.reshape(M, 1, 1)
			img = ifft(fft(img, axis=1) * np.exp(-Mgrid * Ny), axis=1)
		elif shear_axis == 1: 
			Nx = -np.sin(np.deg2rad(theta)) * (ifftshift(np.arange(-np.floor(M/2), np.ceil(M/2)))/M).reshape(1,M,1)
			Ngrid = 2*(1j)*np.pi * (np.arange(N)-np.floor(N/2)+1)
			Ngrid = Ngrid.reshape(N, 1, 1)
			img = ifft(fft(img,axis=0) * np.exp(Ngrid * Nx), axis=0)
		else: print('Shear axis has to be 0 or 1')
